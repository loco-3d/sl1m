...
# [ 1    , 1    , 1      , 1      , 3 * n_moving_feet    , for each moving foot, 0 if n_surfaces == 1, else n_surfaces]
#
...
class Planner:
    def init(self):
        ...
        self.default_n_equality_constraints =

        self.com_xy = self.expression_matrix(2, 0)
        self.com_2 = self.expression_matrix(3, 0)
        self.com_2[2, 2] = 0
        self.com_2[2, 3] = 1
    ...
    def expression_matrix(self, size, j):
        """
    ...
    def foot(self, index):
        """
        Generate a selection matrix for a given foot defined by an index 
        @param index index of the foot to select
        @return a (3, number of default variables (without slacks)) matrix
        """
        return self.expression_matrix(3, 4 + 3 * index)

    def slack_selection_vector(self):
        """
        Get the selection vector of the slack variables
        @return the selection vector
        """
        selection_vector = np.zeros(self.total_n_variables())
        i = 0
        for phase in self.pb.phaseData:
            n_variables_phase = self.phase_n_variables(phase)
            n_feet_variables = self.phase_n_feet_variables(phase)
            n_slacks = n_variables_phase - n_feet_variables - self.default_n_variables
    ...
    def set_problem(self, pb):
        ...
        self.alphas = self.slack_selection_vector()

    def feet_last_moving_phase(self, phase_id):
        """
        ...
        for id, phase in enumerate(self.pb.phaseData):
            if id >= phase_id:
                break
            moving_feet = phase.moving
            feet_last_moving_phase[moving_feet] = id
        return feet_last_moving_phase

    def phase_n_variables(self, phase):
        """
        ...
        n_variables += self.phase_n_feet_variables(phase)
        n_surfaces = phase.n_surfaces
        if n_surfaces > 1:
            n_variables += n_surfaces
        return n_variables

    def phase_n_feet_variables(self, phase):
        """
        Counts the number of feet variables in a phase
        @param phase concerned phase
        @return number of feet variables in the phase
        """
        return 3 * len(phase.moving)

    def total_n_variables(self):
        """
        Counts the number of variables, inequalities constraints in the problem
        @return the number of variables of the problem
        """
        return sum([self.phase_n_variables(phase) for phase in self.pb.phaseData])

    def phase_n_ineq(self, id, phase):
        """
    ...
    def convert_pb_to_LP(self, pb, convert_surfaces=False):
        ...
        n_variables = self.total_n_variables()
        n_ineq = self.total_n_ineq()
        n_eq = self.total_n_eq()
        ...
        for id, phase in enumerate(self.pb.phaseData):
            # inequalities
            j_next = js[-1] + self.phase_n_variables(phase)
            feet_phase = self.feet_last_moving_phase(id)
    ...
    def get_alphas(self, result):
        ...
        j = 0
        for phase in self.pb.phaseData:
            alphas.append(result[j + self.default_n_variables:j + self.phase_n_variables(phase)])
            j += self.phase_n_variables(phase)
        return alphas
