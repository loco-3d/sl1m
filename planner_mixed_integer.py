...
def checkMipValues(res,resMip,slackIndices):    
    zeroFound = False
    lastIdx = 0
    #~ print "slackIndices ", slackIndices
    for i, el in enumerate(slackIndices):
        #~ print "i, el, res ", i, el, res[el]
        if resMip[i] == 0:
            assert res[el] <= 0.01
            zeroFound = True
        #~ if resMip[i] > 0:      
        if i > 0:
            ...
            if el - lastIdx > 2:
                ...
                if i+1 < len(slackIndices):
                    lastIdx = slackIndices[i+1]
    #~ return

def convert_to_cvx(slackMatrix,A,b,E,e):
    rdim = A.shape[1]
    ...
    #~ print "res ", res
    #~ print "MIP VALUES ", tovals(boolvars)
    
    #~ checkMipValues(tovals(varReal),tovals(boolvars),slackIndices)
    
    return tovals(varReal)
...
if __name__ == '__main__':
    ...
    slackMatrix = c
    res2 = convert_to_cvx(slackMatrix,A,b,E,e)
    ...
    ax = draw_scene()
    #~ plotQPRes(pb, res, ax=ax, plot_constraints = False)
    plotQPRes(pb, res2, ax=ax, plot_constraints = False)
